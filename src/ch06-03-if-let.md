...
<mark>`if let` 语法获取通过等号分隔的一个模式和一个表达式。</mark>它的工作方式与 `match` 相同，这里的表达式对应 `match` 而模式则对应第一个分支。在这个例子中，模式是 `Some(max)`，`max` 绑定为 `Some` 中的值。<mark>接着可以在 `if let` 代码块中使用 `max` 了，就跟在对应的 `match` 分支中一样。</mark>模式不匹配时 `if let` 块中的代码不会执行。
